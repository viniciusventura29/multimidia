...
mod tests {
    ...
    #[tokio::test]
    async fn haiku_nao_manda_effort_nem_thinking() {
        let dublê = Dublê::com(vec![fim()]);
        let (agente, _) = agente(dublê.clone(), Config::nova(Modelo::Haiku));
        let _ = agente.rodar("x").await;

        let corpo = &dublê.pedidos()[0];
        // `effort` no Haiku 4.5 é 400, e `thinking` só existia para o Opus.
        assert!(corpo.get("output_config").is_none());
        assert!(corpo.get("thinking").is_none());
        assert_eq!(corpo["model"], "claude-haiku-4-5");
    }
