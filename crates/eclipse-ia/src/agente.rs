...
impl Agente {
    ...
    fn montar_corpo(&self, mensagens: &[Value]) -> Value {
        let mut corpo = json!({
    ...
    pub async fn rodar(&self, pedido: &str) -> Result<Turno, IaError> {
        ...
        while iteracoes < self.config.max_iteracoes {
            ...
            let corpo = self.montar_corpo(&mensagens);
            let resposta = self.transporte.enviar(&corpo, &self.betas).await?;
        ...
        // Estourar o teto sem ter pintado nada é falha de verdade: o turno
        // gastou token e não produziu tela. Se pintou antes de estourar, o que
        // foi pintado vale.
        if !terminou && quadro.is_none() {
